...
fun.clean.nolog.100reads <- prune_taxa(taxa_sums(fun.clean.ss) > 500, fun.clean.nolog)
fun.its1.clean.nolog.100reads <- prune_taxa(taxa_sums(fun.its1.clean.ss) > 1000, fun.its1.clean.nolog)
...
dim(training_set_diagnosis)   # when split ratio 0.75 -> 96
dim(test_set_diagnosis)  # when split ratio 0.25 -> 33
...
##Error rate
colnames(classifier_diagnosis$err.rate) <- c('OOB','Control','RA')
...
cv.bac$error.cv #1340 OTUs
mean(cv.bac$error.cv) #0.07384978
