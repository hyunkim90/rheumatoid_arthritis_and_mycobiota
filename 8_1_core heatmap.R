...
## need to start from 11

## (1) bacteria 99%
## need core - wild -7 otu
...
df.selected.rel <- df.otu.rel %>% filter(OTU %in% core.bac.70)
...
b.add <- merge(excel.bac, bac.list.OTU_id, by ="OTU")
b.order<- b.add %>% dplyr::select(OTU,total, OTU_id)%>% group_by(OTU_id)%>% summarise(Totals=sum(total)) %>% arrange(desc(Totals))
b.order.vec <- b.order$OTU_id

(b.add <- as.data.frame(b.add))
df.rel <- df.selected.rel %>% dplyr::select(OTU,Sample,RelAbundance,Abundance) %>% arrange(desc(Abundance))
df.rel <- merge(df.rel,bac.list.OTU_id,by='OTU')
head(df.rel)
...
plot_core_heatmap <- function(df_bwil, colfunc){
  ...
  ggplot(df_bwil, aes(x=OTU_id, y=Sample, fill=RelAbundance)) +
    geom_tile(colour="black",size=.4) +
    scale_y_discrete(expand=c(0,0), position = "right")+
    scale_x_discrete(expand=c(0,0), position = "bottom")+
    theme_grey(base_size=8)+
    labs(x="",y="",title="Bacterial Core OTUs")+
...
colfunc.bac <- colorRampPalette(c("#FFFFFF","#003366"))
plot_core_heatmap(df.rel, colfunc.bac)
## (2) Fungi prevalence 99%
